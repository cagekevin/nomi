...
> 绘制日期：2026-08-10。**本次复核：2026-08-12**（逐文件核对真实 import 关系后修订）。
>
> 阅读约定：箭头 `A ──▶ B` 表示「A 模块 import 了 B 模块」。实线 = 运行期 import；虚线 = 仅 `import type`（类型层，不参与循环）；被标注「注入」的边表示通过函数参数注入而非 import。

---

## 〇、2026-08-12 复核结论（先看这里）

相比 2026-08-10 版，**整体风险进一步下降**，有两处结构性改进、一处新边补充：

1. **任务类型已抽到叶子 `tasks/taskTypes.ts`**：`runtime.ts` 只 `import type` 自它，`catalog/*`（profileHttpRequest / multipartOperation / customCallDispatch / catalogCommit）、`providerAdapter`、`audioTaskRunner`、`textTaskRunner` 的类型依赖也从 runtime 迁走。**catalog 域对 runtime 的类型反向依赖已清零**。
2. **循环 3 customCallDispatch 从「注入消解」升级为「类型脱钩」**：它现在 `import type` 自 `../tasks/taskTypes`（不再是 `../runtime`），运行期符号仅 `localizeTaskAsset` 经参数注入。类型层 + 运行期层均不再指向 runtime，闭环从结构上移除。
3. **新增单向边 `capabilityCore/* → runtime`**（原版未记录）：`host.ts` / `mcpStdioServer.ts` 运行期 import `runTask, fetchTaskResult`。**runtime 不反向 import capabilityCore / productionRun**，故为单向耦合、不构成循环。

> 现存风险仍是两处，均受控且脆弱：`runtime ↔ catalogStore`（唯一文件级运行期循环）、`runtime ↔ taskResultQuery`（调用期循环）。详见后文。
...
```
 ...
 providerAdapter/verifier.ts · capabilityCore/host.ts · capabilityCore/mcpStdioServer.ts
 tasks/taskResultQuery.ts
```

> 说明：`runtime.ts` 是绝对的依赖汇聚点——几乎所有 runner / IPC 触发的执行路径都从它走。它既向下依赖 `catalogStore` 的纯函数，又被约 13 个模块反向 import，是循环依赖风险的最高发地。作为「类型出口」的负担已减轻：任务/目录类型下放到 `tasks/taskTypes.ts`，`capabilityCore` 走运行期单向边。
...
### 循环 3：runtime ↔ customCallDispatch（已彻底消解）✅ 类型脱钩

> 2026-08-12 复核：相比原版「注入消除」，现更进一步——**类型层也脱钩了**。
...
- `customCallDispatch.ts` 的类型依赖 `import type { TaskRequest, TaskResult } from "../tasks/taskTypes"`（不再是 `../runtime`，见第 19 行）——类型层不指向 runtime。
- `localizeTaskAsset` 由调用方 **作为参数注入**（`CustomCallDispatchInput.localizeTaskAsset`），运行期也不 import runtime。
- `runtime.ts:60` 运行期 import `runCustomCallTask`，但只 import 符号不触发对端加载期求值 → 闭环在类型层与运行期层均已移除。

> 这是处理循环依赖的**推荐范式**（R1 加新必删旧的对照：引入新耦合时优先用「类型 only import + 函数注入」替代运行期 import）。`tasks/taskTypes.ts` 是任务共享类型的叶子出口。
...
| 模块 | import 内容 | 风险级 | 备注 |
|------|------------|--------|------|
| `textTaskRunner.ts` | `billingKindForTaskKind, findExecutableModelForTask` | 🟡 中 | `TaskRequest/Result` 已改从 `tasks/taskTypes` 引，运行期符号有限 |
| `audioTaskRunner.ts` | `buildProfileHttpRequest` | 🟡 中 | `TaskRequest/Result` 已从 `tasks/taskTypes` 引 |
...
| `ai/agentStreamConsumer.ts` | `type AgentChatV2Hooks` | 🟢 低 | 纯类型（实测确认） |
| `providerAdapter/verifier.ts` | `buildProfileTaskResult, executeProfileOperation, type TaskRequest, type TaskResult` | 🟡 中 | 运行期符号确认存在，但仅在函数体内使用（单向） |
| `capabilityCore/host.ts` | `runTask, fetchTaskResult` | 🟡 中 | 2026-08-12 复核新增。headless 执行体，单向边（runtime 不反向 import 它） |
| `capabilityCore/mcpStdioServer.ts` | `runTask, fetchTaskResult` | 🟡 中 | 同上，单向边 |
| `catalog/catalogCommit.ts` | `type TaskRequest` | 🟢 低 | 纯类型（自 `tasks/taskTypes`） |
| `catalog/profileHttpRequest.ts` | `type TaskRequest` | 🟢 低 | 纯类型 |
| `catalog/multipartOperation.ts` | `type TaskRequest` | 🟢 低 | 纯类型 |
| `catalog/customCallDispatch.ts` | 无（类型自 `tasks/taskTypes`，运行期全注入） | 🟢 低 | 已彻底消解（见循环 3） |
...
4. **拆壳即消环**：巨壳拆分（R12）时将「被两边都需要的纯函数」下沉到独立叶子模块（如 `runtimePaths`、`jsonUtils`、`catalog/types`、`tasks/taskTypes`），让原两边改为依赖叶子，从结构上破环。**`tasks/taskTypes.ts` 即此范式的成功样本**（2026-08-12 确认）：任务类型从 runtime 抽离后，catalog 域对 runtime 的类型反向依赖清零。

---

## 五、一句话结论

Nomi 后端的循环依赖**真实存在且集中于 `runtime.ts` 中枢**：`runtime ↔ catalogStore` 是唯一文件级运行期循环，`runtime ↔ taskResultQuery` 是调用期循环，二者均靠「函数体内取值 + re-export 隔离」维持安全。相比 2026-08-10 版，**整体风险已下降**：任务类型抽到叶子 `tasks/taskTypes.ts`，`customCallDispatch` 彻底类型脱钩，`capabilityCore` 新增边为单向不构成循环。现存风险**受控但脆弱**——任何把跨边引用移进加载期顶层求值的改动都会重新引爆 `runtime ↔ catalogStore` 死锁，建议补一条加载期不变量测试固化当前安全边界（对应工程纪律 P2 修根因、R9 防巨壳）。
