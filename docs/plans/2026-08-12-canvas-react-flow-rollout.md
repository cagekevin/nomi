...
### 进行中 / 下一步
- **✅ S8 测试迁移 + 孤儿清理完成（2026-08-12）**：删老画布残留孤儿（ + 连带 test）；registry component 改指向 ReactFlowNode（死配置，unknown 桥接）。**保留** canvasPointerGestureModel/canvasControlsHelpModel（被 CanvasControlsHelpPopover→CanvasNavigationStack 活跃使用）。typecheck/lint(94w)/test(4100)/smoke(13)/parity(23)/build 全绿。**F9 连整组真机补验标注遗留专项**（代码就位，完整 UI 真机需 group fixture）。S8 plan 见 `docs/plans/2026-08-12-canvas-react-flow-s8-test-migration-cleanup.md`。
- **✅ S7 删老画布 + H 域契约迁移完成（2026-08-12）**：H6 挂载点切 `<ReactFlowGenerationCanvas/>`（NomiStudioApp 删 renderCanvasWithReactFlow 开关 + GenerationCanvas lazy）+ 删 `GenerationCanvas.tsx` 组件 + 连带删 `canvasControlsStructure.test.ts`（绑老自研渲染层实现的结构契约测试）。**关键洞察**：ReactFlowNode 复用 `.generation-canvas-v2-node` class + `data-node-id`，H1-H3（onboarding/保存提示/错误浮层）的节点命中选择器在 react-flow 下自动满足；H1 子元素 `.generation-canvas-v2-node [aria-label="生成素材"]` 由 NodeGenerationComposer 的 generateAsset 提供（已命中）。H7 反向依赖扫描通过（import GenerationCanvas 仅 NomiStudioApp）。typecheck/lint(96w)/test(4105)/smoke(13)/parity(23) 全绿。**孤儿 hook（useCanvasViewport/useCanvasPointerInteractions/useCanvasContextNodeMenu 等）留 S8 清**（plan 评审点 4 保守边界）。
- **✅ S6 批量生产完成（2026-08-12）**：C3 多选工具条（CanvasSelectionToolbar 复用，flow→容器屏幕坐标定位）+ C4 批量 dock（CanvasBatchGenerateDock 复用）+ E1 成组/解组 + E2 批量（useCanvasGroupActions/useCanvasProductionActions 复用）+ D5 快捷键（useCanvasShortcuts 复用，stageRef→canvasRef，deleteKeyCode=Delete 内建）+ G2 聚焦（FOCUS 事件→setCenter）+ readOnly 透传（NodeReadOnlyContext）。typecheck/lint/test(4121)/smoke(13)/parity(23) 全绿。S6 plan 见 `docs/plans/2026-08-12-canvas-react-flow-s6-batch-production.md`。**F9 连整组真机补验入口已就位（成组按钮），S7 前补真机验证**。
- **✅ S5 视口与 LOD 完成（2026-08-12）**：B1 变换同步（useOnViewportChange → store.canvasZoom/Offset，平移节流 100ms + onEnd 落定 + 回环 guard）+ B2 多分类 viewport 记忆（workbenchStore categoryViewports，onEnd 记 + 切分类 setViewport 恢复）+ B3 自动 fit（首载/切分类无记忆 fitView）+ C5 minimap/缩放条（复用 CanvasNavigationStack，坐标来自 store 镜像，回调接 fitView/setViewport/zoomTo/setCenter/tidy）+ F7 LOD（onlyRenderVisibleElements）+ G4（fit 覆盖）+ G5（tidy）。typecheck/lint/test(4121)/smoke(13)/parity(23) 全绿。S5 plan 见 `docs/plans/2026-08-12-canvas-react-flow-s5-viewport-lod.md`。
