  ...
  milestones: [
    {
      ...
      async verify(ctx) {
        const toolbar = await Promise.all(["创作", "生成", "预览", "去出片"].map((name) => ctx.win.getByRole("button", { name, exact: false }).first().isVisible().catch(() => false)));
        return [
    ...
    },
    {
      ...
      async act(ctx) {
        const cta = ctx.win.locator('button[aria-label="新建一个画面节点"]').first();
        await cta.click({ timeout: 5000 });
      },
      async verify(ctx) {
        const emptyCtaVisible = await ctx.win.locator('button[aria-label="新建一个画面节点"]').first().isVisible().catch(() => false);
