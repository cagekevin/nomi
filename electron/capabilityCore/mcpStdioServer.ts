...
// 取代旧 scripts/nomi-mcp.mjs 的 MCP 路径（MCP 协议内联进二进制，无 node 依赖、入口永远存在，P1）。
// 注：scripts/lib/nomiClient.mjs 的 CLI 传输层（A/B 自适应 invoke）仍保留——它是 scripts/nomi.mjs 与评测脚本的共用客户端，本模块只接管 MCP stdio 那一侧。
